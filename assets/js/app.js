class UserApp {
	...
	reloadGrid() {
		...
		$.ajax({
			...
			success: function(response) {
				if (response !== '') {
					// To prevent "Cannot reinitialise DataTable" error,
					// we must destroy the old instance first.
					if ($.fn.DataTable.isDataTable('#users_grid')) {
						$('#users_grid').DataTable().destroy();
					}
...
$(document).ready(function() {
	// Initialize DataTable on the users page if the grid exists.
	if ($('#users_grid').length) {
		$('#users_grid').DataTable();
	}
});
